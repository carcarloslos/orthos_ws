...
void swivelFeedbackCallback(const orthos_servo_driver::ServoControllerFeedback::ConstPtr& msg)
{
    ...
    swivel_lower.setOrigin(tf::Vector3(0, 0, 0.033)); // 3 cm upward
    tf::Quaternion swivel_lower_rotation;
    swivel_lower_rotation.setRPY(0, 0, msg->actual_servo_1*-d2r); // Rotate around Z-axis
    ...
    swivel_upper_rotation.setRPY(0, msg->actual_servo_2*-d2r, 0); // Rotate around Y-axis
    ...
    zed_camera_center.setOrigin(tf::Vector3(0, 0, 0.063)); // 3 cm along z-axis
    ...
    br.sendTransform(tf::StampedTransform(zed_camera_center, ros::Time::now(), "swivel_upper", "zed2_camera_center"));
}
...
int main(int argc, char** argv)
{
    ...
    ros::Subscriber sub = nh.subscribe("information/orthos_swivel/feedback", 1, swivelFeedbackCallback);
