...
ROS_INFO("Service /arm/home_wrist called with value: %d", req.value);
    ...
    if (hall == false) {
        res.success = false;
        ROS_INFO("Not in sensor range!!");
    	return false;
    }  
    else {
        orthos_cubemars_driver::CubeMarsPosVelGoal goal;
        goal.position_goal = wristposition + 60;
        goal.speed_goal = 0;
        goal.accel_goal = 0;
        goal.control_mode = 0;

        while (hall == true) {
            ROS_INFO("goal.position_goal: %f", goal.position_goal);
            ROS_INFO("goal.position_goal: %i", hall);
        ...
        ROS_INFO("done");
        goal.position_goal = wristposition;
        goal.speed_goal = 0;
        goal.accel_goal = 0;
        goal.control_mode = 0;
        motor_wrst.UpdateGoalState(goal);
...
// Callback function for the wrst current command subscriber
void wrstCurrentCommandCallback(const orthos_cubemars_driver::CubeMarsCurrentLoopMsg::ConstPtr& current_cmd_msg) {
    // Update the goal state for motor_wrst
    motor_wrst.CurrentLoopModeCommand(*current_cmd_msg);
}
...
//----// New current mode subscriber for wrist
    ros::Subscriber wrst_current_cmd_sub = nh.subscribe<orthos_cubemars_driver::CubeMarsCurrentLoopMsg>(
        "arm/wrst/current_cmd", 10, wrstCurrentCommandCallback);
